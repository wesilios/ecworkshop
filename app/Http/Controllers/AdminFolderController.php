    ...
    public function create(Request $request)
    {
    	if($request->ajax()) {
            ...
            else
            {
                ...
                {
                    $folder_temp = Folder::find($folder_id_parent);
...
//    	$folder_string = [['folder_id' => '1','folder_name'=>'root','folder_slug'=>'root']];
    ...
    public function ajaxSliderModalShow(Request $request)
    {
        if($request->ajax())
        {
            ...
            $data = view('admin.ajax.media.slider.slider', compact('medias','folder','folder_list','folder_string','slider'))->render();
            return response()->json(['option'=>$data, 'folder_id' => $folder->id, 'folder_string' =>$folder_string ]);
        }
    }

    public function createSliderAjax(Request $request)
    {
        if($request->ajax()) {
            $validator = Validator::make($request->all(), [
                'folder_name' => 'required|string|max:255'
            ...
            $folder = Folder::find($request->folder_id);
            $folder_list = Folder::where('folder_id',$request->folder_id)->get();
            $data = view('admin.ajax.media.slider.new_folder',compact('folder','folder_list', 'slider'))->render();
            return response()->json(['option'=>$data]);
        }
    }
}
