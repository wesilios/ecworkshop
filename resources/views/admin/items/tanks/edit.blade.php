...
@section('content')
	...
	<section class="content">
        <div class="row">
        	...
        	{!! Form::open(['method'=>'PUT', 'action'=>["AdminTanksController@update", $tank->id], 'class'=>'form-horizontal', 'files'=>true]) !!}
				<div class="col-md-8">
					<!-- Horizontal Form -->
					<div class="box">
		                <!-- form start -->
	                	<div class="box-body">
	                		<div class="col-md-12">
	                			<div class="form-group {{ $errors->has('name') ? ' has-error' : '' }}">
									...
									{!! Form::text('name', $tank->item->name, ['class'=>'form-control', 'placeholder'=>'Insert name']) !!}
		                      		@if ($errors->has('name'))
			                    ...
			                    <div class="form-group {{ $errors->has('price') ? ' has-error' : '' }}">
									...
									{!! Form::number('price', $tank->item->price, ['class'=>'form-control', 'placeholder'=>'Insert price']) !!}
		                      		@if ($errors->has('price'))
						                <span class="help-block">
			                    ...
			                    <div class="form-group {{ $errors->has('price_off') ? ' has-error' : '' }}">
									...
									{!! Form::number('price_off', $tank->item->price_off, ['class'=>'form-control', 'placeholder'=>'']) !!}
			                    ...
			                    <div class="form-group {{ $errors->has('summary') ? ' has-error' : '' }}">
									...
									{!! Form::textarea('summary', $tank->item->summary, ['class'=>'form-control', 'rows'=>'3']) !!}
			                    ...
			                    <div class="form-group {{ $errors->has('description') ? ' has-error' : '' }}">
									...
									{!! Form::textarea('description', $tank->item->description, ['class'=>'form-control textarea']) !!}
				...
				<div class="col-md-4">
					<div class="box">
						<div class="box-body">
							<div><strong>Ngày đăng:</strong> {{ date("F jS, Y", strtotime($tank->item->created_at)) }}</div>
							<div><strong>Ngày cập nhật:</strong>  {{ date("F jS, Y", strtotime($tank->item->updated_at)) }}</div>
							<div><strong>Đăng bởi:</strong>  {{ $tank->item->admin->name }}</div>
						</div>
								...
								<div class="form-group">
									<div class="checkbox">
					                    <label class="control-label">
					                      	<input type="checkbox" class="minimal" {{ $tank->homepage_active==1 ? 'checked' : '' }} name="homepage_active">
					                      	Hiển thị trang chủ
					                    </label>
	            			...
	            			<div style="min-height:600px; width:100%">
	                			<div class="nav-tabs-custom" style="height:100%">
					                <ul class="nav nav-tabs">
					                  	...
					                  	<li {{ $tank->medias->isNotEmpty() == 1 ? '' : 'class=active' }}><a href="#tab_2" data-toggle="tab">Media gallery</a></li>
					                  	<li {{ $tank->medias->isNotEmpty() == 1 ? 'class=active' : '' }}><a href="#tab_3" data-toggle="tab">Ảnh sản phẩm</a></li>
					                ...
					                <div class="tab-content">
							        		...
							        		<div class="box box-hidden">
												<div class="box-body">
													<div class="form-hidden">
									    				{!! Form::open(['method'=>'PUT', 'action'=>["AdminTanksController@uploadImage",$tank->id] ,'files'=>true]) !!}
							        	...
							        	<div class="tab-pane {{ $tank->medias->isNotEmpty() == 1 ? '' : 'active' }}" id="tab_2">
							        		<div class="box">
							        			<div class="box-body" >
							        	...
							        	<div class="tab-pane {{ $tank->medias->isNotEmpty() == 1 ? 'active' : '' }}" id="tab_3">
							        		<div class="box">
							        			<div class="box-body" >
													{!! Form::open(['method'=>'PUT', 'action'=>["AdminTanksController@set_image_index",$tank->id]]) !!}
														<div class="form-group">
								                      		<select class="form-control" name="media_id" id="selIndexImgForm" style="">
																@foreach($tank->medias as $media)
									                        		<option value="{{ $media->id }}">{{ $media->file_name }}</option>
									                        	@endforeach
									                      	</select>
		  	...
		  	<div class="modal fade item_modal" id="delete{{ $media->id }}" role="dialog">
		    	<div class="modal-dialog delete-dialog" style="">
		      		<div class="modal-content">
		        		...
		        		<div class="modal-footer">
		        			{!! Form::open(['method'=>'DELETE', 'action'=>['AdminTanksController@delete_image', $tank->id], 'class'=>'form-horizontal']) !!}
