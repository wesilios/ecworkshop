	       					...
	       					<div class="table-responsive col-sm-12">
		       					<table class="table table-hover order_table">
				                    ...
				                    @php
										$i = 1
			                    	@endphp
			                    	@if(isset($customer) && isset($customer->orders))
...
@section('extendscripts')
	<script>
		$('.order_status').click(function(e) {
			...
			$.ajax({
                url: "{{ route('admin.customer.orders.status') }}",
                method:'POST',
                dataType:'json',
                data: {status:status, customer_id:customer_id, _token:token},
		...
		$("input[name='search_query']").on('keyup', function() {
			...
			var customer_id = $("input[name='customer_id']").val();
			$.ajax({
				url: "{{ route('admin.customer.order.search') }}",
				method: 'GET',
				dataType:'json',
